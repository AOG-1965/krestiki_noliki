...
def on_click(row, col):         # для передачи значения переменных row и col).
    ...
    winner = check_winner()
    if winner == "X" or winner == "0":          # В случае выявления победителя вызывается сообщение
        messagebox.showinfo("Игра окончена", f"Игрок {current_player} победил!")
        return                                  # и игра останавливается
    elif winner == "Draw":
...
def check_winner():
    for i in range(3):  # Три циклические итерации проверки по горизонтали
        if buttons[i][0]["text"] == buttons[i][1]["text"] == buttons[i][2]["text"] != "":
            return buttons[i][0]["text"]    # В случае совпадения трех кнопок по горизонтали и они не пустые
                                            # фиксируется победа ("X" или "O")
        if buttons[0][i]["text"] == buttons[1][i]["text"] == buttons[2][i]["text"] != "":
            return buttons[0][i]["text"]    # В случае совпадения трех кнопок по вертикали и онт не пустые
                                            # фиксируется победа ("X" или "O")
    if buttons[0][0]["text"] == buttons[1][1]["text"] == buttons[2][2]["text"] != "":
        return buttons[0][0]["text"]        # В случае совпадения трех кнопок по диагонали (слева-направо)
                                            # и они не пустые, фиксируется победа ("X" или "O")
    if buttons[0][2]["text"] == buttons[1][1]["text"] == buttons[2][0]["text"] != "":
        return buttons[0][2]["text"]        # В случае совпадения трех кнопок по диагонали (справа-налево)
                                            # и они не пустые, фиксируется победа ("X" или "O")

    # Проверка ничьи: если нет пустых клеток и победителя
    if all(buttons[i][j]["text"] != "" for i in range(3) for j in range(3)):
        return "Draw"                       # Фиксирование "ничью"

    return None  # Если никто не выиграл и клетки еще есть, продолжаем игру со сменой игрока
